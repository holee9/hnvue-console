...
/// <summary>
/// Builds conformant DICOM files for the Digital X-Ray Image (DX) For Presentation IOD.
/// SOP Class UID: 1.2.840.10008.5.1.4.1.1.1.1 (Digital X-Ray Image Storage - For Presentation)
///
/// Implements mandatory Type 1 and Type 2 attributes per DICOM PS 3.3 C.8.11.1 (DX IOD).
/// </summary>
public sealed class DxImageBuilder : IImageBuilder<DicomImageData>
{
    private readonly ILogger<DxImageBuilder> _logger;

    /// <summary>SOP Class UID for DX Image Storage - For Presentation.</summary>
    public static readonly DicomUID DxForPresentationSopClass =
        DicomUID.Parse("1.2.840.10008.5.1.4.1.1.1.1");

    /// <summary>
    /// Initializes a new instance of the <see cref="DxImageBuilder"/> class.
    /// </summary>
    /// <param name="logger">Logger for build diagnostics.</param>
    public DxImageBuilder(ILogger<DxImageBuilder> logger)
    {
        _logger = logger;
    }
    ...
    public DicomFile Build(DicomImageData imageData)
    {
        ...
        _logger.LogDebug(
            "Building DX IOD: Rows={Rows}, Columns={Columns}, SopUid={SopUid}",
            imageData.Rows,
            imageData.Columns,
        ...
        PopulateDxSeriesModule(dataset, imageData);
        PopulateGeneralImageModule(dataset, imageData);
        PopulateDxImageModule(dataset, imageData);
        PopulateImagePixelModule(dataset, imageData);
        PopulateXRayAcquisitionModule(dataset, imageData);

        var file = new DicomFile(dataset);

        _logger.LogDebug("DX IOD built successfully for SopUid={SopUid}", imageData.SopInstanceUid);
    ...
    private static void PopulateSopCommonModule(DicomDataset dataset, DicomImageData imageData)
    {
        ...
        dataset.Add(DicomTag.SOPClassUID, DxForPresentationSopClass);

        // SOP Instance UID (0008,0018) - Type 1
        dataset.Add(DicomTag.SOPInstanceUID, imageData.SopInstanceUid);

        // Specific Character Set (0008,0005) - Type 1C: Use ISO_IR 6 (default ASCII repertoire)
    ...
    {
        // Modality (0008,0060) - Type 1: Must be "DX"
    ...
    private static void PopulateDxSeriesModule(DicomDataset dataset, DicomImageData imageData)
    {
        // Body Part Examined (0018,0015) - Type 2
        dataset.Add(DicomTag.BodyPartExamined, imageData.BodyPartExamined ?? string.Empty);

        // Laterality (0020,0060) - Type 2C
        dataset.Add(DicomTag.Laterality, string.Empty);
    }
    ...
    private static void PopulateGeneralImageModule(DicomDataset dataset, DicomImageData imageData)
    {
        ...
        // Image Type (0008,0008) - Type 1: Must include ORIGINAL\PRIMARY for DX
        dataset.Add(DicomTag.ImageType, new[] { "ORIGINAL", "PRIMARY" });
    }

    private static void PopulateDxImageModule(DicomDataset dataset, DicomImageData imageData)
    {
        // Presentation Intent Type (0008,0068) - Type 1
        dataset.Add(DicomTag.PresentationIntentType, imageData.PresentationIntentType);

        // Burned In Annotation (0008,2111) - Type 1: "NO" for DX For Presentation
        dataset.Add(DicomTag.BurnedInAnnotation, "NO");

        // Pixel Spacing (0028,0030) - Type 1C: required when Imager Pixel Spacing absent
        if (imageData.PixelSpacing.HasValue)
        {
            dataset.Add(DicomTag.PixelSpacing,
                new[] { (decimal)imageData.PixelSpacing.Value.RowSpacingMm, (decimal)imageData.PixelSpacing.Value.ColumnSpacingMm });
        }
        else
        {
            // Set empty when no calibration data available
            dataset.Add(DicomTag.PixelSpacing, Array.Empty<decimal>());
        }

        // Imager Pixel Spacing (0018,1164) - Type 3
        if (imageData.ImagerPixelSpacing.HasValue)
        {
            dataset.Add(DicomTag.ImagerPixelSpacing,
                new[] { (decimal)imageData.ImagerPixelSpacing.Value.RowSpacingMm, (decimal)imageData.ImagerPixelSpacing.Value.ColumnSpacingMm });
        }
    }
    ...
    private static void PopulateXRayAcquisitionModule(DicomDataset dataset, DicomImageData imageData)
    {
        ...
        {
            dataset.Add(DicomTag.ExposureInuAs, (int)(imageData.ExposureInMas.Value * 1000)); // Convert mAs to uAs
        }
