...
본 문서는 FDA 및 MFDS 요청 또는 감사 시, 허가된 버전의 HnVue Console SW를 정확히 재현할 수 있도록 소스코드 위치와 빌드 환경 전체를 문서화한다.  
소스코드 자체를 포함하지 않으며, 소스코드의 위치·접근 방법·재현 가능한 빌드 절차의 명세서로서 기능한다.  
형상관리 계획서(HnVue-A05)와 연계하여 모든 릴리즈 버전의 소스코드 및 빌드 환경을 추적한다.
...
| 문서 번호 | 문서명 | 관계 |
|-----------|--------|------|
| HnVue-A05 | 형상관리 계획서 (CMP) | 형상 항목 관리 기준 |
| HnVue-A04 | SOUP 목록 | 의존성 라이브러리 버전 참조 |
| HnVue-A06 | SW 릴리즈 기록 | 릴리즈별 빌드 정보 연계 |
...
| 브랜칭 전략 | Git Flow (HnVue-A05 §4.3.2 참조) | - |
| 태그 정책 | `v[MAJOR.MINOR.PATCH]` (예: v1.0.0) | HnVue-A05 §4.3.4 |
...
| 항목 | 내용 |
|------|------|
| 저장소 위치 (URL) | [작성 필요 — 예: https://github.com/h-abyz/hnvue] |
...
- 의존성 업데이트는 변경 통제 프로세스(HnVue-A05 §4.5)를 통해서만 수행한다.
...
# 3. 빌드 실행 (Release 모드)
[빌드 명령어 — 작성 필요]
# 예: msbuild HnVue.sln /p:Configuration=Release /p:Platform=x64
...
# 5. 설치 패키지 생성
[작성 필요]
# 예: msbuild HnVueSetup.wixproj /p:Configuration=Release

# 6. 코드 서명
signtool sign /fd SHA256 /t [타임스탬프 서버] HnVue-v1.0.0-setup.msi

# 7. SHA-256 해시 생성 및 검증
certutil -hashfile HnVue-v1.0.0-setup.msi SHA256
# 예상 해시: [작성 필요]
```
...
|---------|--------|------|-------------|
| 실행 파일 | HnVue.exe | v[작성 필요] | [작성 필요] |
| 설치 패키지 | HnVue-v[X.X.X]-setup.msi | v[작성 필요] | [작성 필요] |
| 코드서명 확인 | signtool verify /pa HnVue-v[X.X.X]-setup.msi | - | - |
